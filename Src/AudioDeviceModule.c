...
PRIVATE
mrb_value is_recording(mrb_state *mrb, mrb_value self) {
    UNUSED_ARGUMENT mrb;
    UNUSED_ARGUMENT self;
...
void DefineAudioDeviceModule(mrb_state *mrb, SDL_Event *event) {
    ...
    mrb_define_module_function(mrb, audio_dev, "timestamp",  get_timestamp, MRB_ARGS_NONE());
    mrb_define_module_function(mrb, audio_dev, "which",      get_which,     MRB_ARGS_NONE());
    mrb_define_module_function(mrb, audio_dev, "recording?", is_recording,  MRB_ARGS_NONE());

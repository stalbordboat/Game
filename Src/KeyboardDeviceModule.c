...
// Description: Event Queue Keyboard Device Data Structure
#include "Internal.h"
...
void DefineKeyboardDeviceModule(mrb_state *mrb, SDL_Event *event) {
    ...
    mrb_define_module_function(mrb, keyboard_dev, "timestamp", self_get_timestamp, MRB_ARGS_NONE());
    mrb_define_module_function(mrb, keyboard_dev, "which",     self_get_which,     MRB_ARGS_NONE());
